...
class ScanModeSelector(QGroupBox):
    ...
    def _build(self):
        ...
        self.radio_single = QRadioButton(
            "Single file (find duplicates within one file)"
        )
        self.radio_single.setChecked(True)
        self.radio_single.toggled.connect(
            lambda checked: self._on_toggle("single") if checked else None
        ...
        self.radio_multi = QRadioButton(
            "Multiple files (compare across files)"
        )
        self.radio_multi.toggled.connect(
            lambda checked: self._on_toggle("multiple") if checked else None
        )
...
class AnalysisPage(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._log_timer = QTimer(self)
        self._log_timer.setInterval(100)
        self._log_timer.timeout.connect(
            self._flush_logs
        )
        self._log_timer.start()
    ...
    def _build(self):
        ...
        self.no_project_state = EmptyState(
            title="No project selected",
            message=(
                "Select a project or go to Projects page "
                "to create one."
            ),
        )

        outer.addWidget(
            self.no_project_state
        )
        ...
        self.import_panel.files_dropped.connect(
    ...
    def _load_project_list(
        ...
        self.project_combo.blockSignals(
            True
        )
    ...
    def set_project(
        ...
        self._project_data = get_project(
            project_id
        )
        ...
        self.no_project_state.setVisible(
            False
        )

        self.mode_selector.setVisible(
            True
        )
    ...
    def _update_run_hint(self):
        ...
        if count == 0:
            self.status_hint_lbl.setText(
                "Import files to begin analysis."
            )
            ...
            self.status_hint_lbl.setText(
                "Ready to find duplicates within this file."
            )
            ...
            self.status_hint_lbl.setText(
                f"Single mode: only first file will "
                f"be scanned. Switch to Multiple mode "
                f"to compare all {count} files."
            )

            self.run_btn.setEnabled(True)

        elif (
            self._scan_mode == "multiple"
            and count < 2
        ):
            self.status_hint_lbl.setText(
                "Multiple mode needs at least 2 files. "
                "Import more or switch to Single mode."
            )
